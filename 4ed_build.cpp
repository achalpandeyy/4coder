...
char *g_includes[] = { "src/custom", FOREIGN "/freetype2", 0, };

//
// Platform layer file tables
//

char *windows_platform_layer[] = { "src/platform_win32/win32_4ed.cpp", 0 };
char *linux_platform_layer[] = { "src/platform_linux/linux_4ed.cpp", 0 };
char *mac_platform_layer[] = { "src/platform_mac/mac_4ed.mm", 0 };
...
// TODO(achal): I don't think there is such a thing as "platform_all" anymore
...
#define CL_LIBS_X64 CL_LIBS_COMMON "..\\ext\\x64\\freetype.lib"
#define CL_LIBS_X86 CL_LIBS_COMMON "..\\ext\\x86\\freetype.lib"

#define CL_ICON "..\\data\\res\\icon.res"
...
build_and_run(Arena *arena, char *cdir, char *filename, char *name, u32 flags){
    ...
    
    {
        ...
        build(arena, flags, Arch_X64, cdir, file, dir, name, get_defines_from_flags(arena, flags), 0, g_includes);
    }
...
buildsuper(Arena *arena, char *cdir, char *file, u32 arch){
    ...
    char *build_script = fm_str(arena, "src/custom/buildsuper_", arch_names[arch], build_script_postfix, BAT);
...
build_main(Arena *arena, char *cdir, b32 update_local_theme, u32 flags, u32 arch){
    ...
    {
        char *file = fm_str(arena, "src/4ed_app_target.cpp");
        char **exports = fm_list_one_item(arena, "app_get_functions");
        
        char **build_includes = g_includes;
    ...
    {
        // TODO(achal): I am not sure if I need platform_includes here anymore?
        char **inc = (char**)fm_list(arena, g_includes, g_src_include, platform_includes[This_OS][This_Compiler]);
    ...
    if (update_local_theme){
        ...
        char *source_themes_folder = fm_str(arena, "data/themes");
...
#error We don't support packaging yet.
    ...
    // copy the fonts
    {
        // NOTE(achal): At this point we are in the 4coder root directory.
        char *fonts_folder = fm_str(arena, "build/fonts");
        char *source_fonts_folder = fm_str(arena, "data/fonts");
        fm_clear_folder(fonts_folder);
        fm_make_folder_if_missing(arena, fonts_folder);
        fm_copy_all(source_fonts_folder, fonts_folder);
    }
    
    // copy bindings.4coder
    {
        char *src = fm_str(arena, "data/bindings.4coder");
        char *dst = fm_str(arena, "build/bindings.4coder");
        fm_copy_file(src, dst);
    }
    
    // copy config.4coder
    {
        char *src = fm_str(arena, "data/config.4coder");
        char *dst = fm_str(arena, "build/config.4coder");
        fm_copy_file(src, dst);
    }
    
    // NOTE(achal): The distribution on itch.io comes with all of these files, which does not automatically come into the build folder:
...
standard_build(Arena *arena, char *cdir, u32 flags, u32 arch){
    buildsuper(arena, cdir, fm_str(arena, g_default_custom_target), arch);
